...
sns.heatmap(
    data=corr_mtrx,linewidth=.2,cmap='RdBu_r',xticklabels=corr_words,yticklabels=corr_words)
plt.tight_layout()
plt.savefig(os.path.join(figPath,'w2v_heatmap_glov.tif'))
plt.close('all')

#
anim = list(set(data_file.loc[data_file['cate']=='Animals','name'].tolist()))
obj = list(set(data_file.loc[data_file['cate']=='Objects','name'].tolist()))
# for word in miss_words:
#     if word in anim:
#         anim.remove(word)
#     else:
...
def plotTsne2D(wVectors1,wVectors2,wlist1,wlist2):
    ...
    #             xytext=(xy_loc[n,0],xy_loc[n,1]),textcoords='offset points')
