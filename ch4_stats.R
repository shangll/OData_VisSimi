setwd("C:\\Users\\u759254\\OneDrive - Radboud Universiteit\\RU-Drive\\DCC\\ch4_LSTM")
p_crit <- 0.05
sizes <- c(1,2,4,8)
...
# 1.1 mean data
#
exp.mean <- read.csv("exp_mean.csv")
...
for (exp.tag in exp.tags){
  ...
  w_obs_rt <- subset(exp.data,(setsize==8)&(cond=="within"))$rt
  b_obs_rt <- subset(exp.data,(setsize==8)&(cond=="between"))$rt
  w_16_lm <- subset(exp.data,(setsize==8)&(cond=="within"))$lm
  w_16_log <- subset(exp.data,(setsize==8)&(cond=="within"))$log
  b_16_lm <- subset(exp.data,(setsize==8)&(cond=="between"))$lm
  b_16_log <- subset(exp.data,(setsize==8)&(cond=="between"))$log
  
  w_resid_lm <- abs(w_obs_rt-w_16_lm)
  w_resid_log <- abs(w_obs_rt-w_16_log)
  b_resid_lm <- abs(b_obs_rt-b_16_lm)
  b_resid_log <- abs(b_obs_rt-b_16_log)
