...
if save_tag==1:
    exp1b_simi.to_csv(
        os.path.join(
            alex_output,'exp1b_simi.csv'),mode='w',header=True,index=False)
print('--- --- --- --- --- ---')
...
if save_tag==1:
    exp2_simi.to_csv(
        os.path.join(
            alex_output,'exp2_simi.csv'),mode='w',header=True,index=False)
print('--- --- --- --- --- ---')
...
exp_simi = pd.concat(
    [exp1b_simi[final_col],exp2_simi[final_col]],axis=0,ignore_index=True)
exp_simi.rename(columns={'cond':'cate'},inplace=True)
if save_tag==1:
    exp_simi.to_csv(
            os.path.join(
                alex_output,'expAll_simi_raw.csv'),mode='w',header=True,index=False)
#
...
exp_simi['corr'] = 'max'
exp_simi = pd.concat([exp_simi,exp_simi_temp],axis=0,ignore_index=True)
...
h,_ = ax[0].get_legend_handles_labels()
ax[0].legend(
    h,['Category','within','between','Correlation','max','mean'],
