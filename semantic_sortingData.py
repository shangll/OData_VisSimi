...
simi_mtrx = pd.read_csv(os.path.join(sgpt_filepath,'img_w2v_simi.csv'))
#
exp1b = pd.read_csv(
...
exp1b_copy['sgpt_mean'] = exp1b_copy[simi_cols].mean(axis=1)
exp1b_copy['sgpt_max'] = exp1b_copy[simi_cols].max(axis=1)
...
exp1b_distr.to_csv(
    os.path.join(
        sgpt_filepath,'exp1b_simi_%s.csv'%file_tag),mode='w',header=True,index=False)
...
for n in range(exp2_copy.shape[0]):
    for k in range(1,9):
        ...
        exp2_copy.loc[n,'simi_val_%d'%k] = simi_mtrx.loc[
                    simi_mtrx['image']==targImg,distrImg].values[0]
exp2_copy['sgpt_mean'] = exp2_copy[simi_cols].mean(axis=1)
exp2_copy['sgpt_max'] = exp2_copy[simi_cols].max(axis=1)
...
exp2_distr.to_csv(
    os.path.join(
        sgpt_filepath,'exp2_simi_%s.csv'%file_tag),mode='w',header=True,index=False)
